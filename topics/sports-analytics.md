...
- **Synthetic data for low-data jersey number detection**: Bhargavi et al. (Frontiers in AI 2022) demonstrate that simple two-stage synthetic data generation (solid-background digits → digits on COCO images) with curriculum learning improves jersey number classification accuracy by 9% on a highly imbalanced American football dataset with only 3,000 labeled samples. The three-step pipeline (CenterNet → AlphaPose → CNN ensemble) achieves 89.3% accuracy with 4.1M parameters, comparable to methods using 10× more parameters and data.
- **Multi-label digit decomposition**: The multi-label (digit-wise) formulation enables compositional generalization to unseen number combinations, an insight that prefigures Grad's (2025) digit-compositional heads. However, the multi-class model still achieves higher absolute accuracy on most numbers.
...
- **Detection vs. recognition vs. single-stage**: Four paradigms now exist — detection-based (Liu & Bhanu 2019), synthetic-data-augmented CNN ensemble (Bhargavi et al. 2022), STR-based (Koshkina & Elder 2024), and single-stage uncertainty-aware (Grad 2025). Bhargavi et al.'s curriculum-learning approach with synthetic data sits between the detection-based and STR-based eras, demonstrating that pose-based localization + simple CNNs can be competitive when data augmentation is well-designed. Grad's single-stage approach surpasses Koshkina on the SoccerNet Challenge set (85.62% vs 79.31%) but no paper compares all four paradigms head-to-head
...
5. [[papers/2022/2022-jersey-number-detection-synthetic-data/2022-jersey-number-detection-synthetic-data|Jersey Number Detection Using Synthetic Data (Frontiers in AI 2022)]] — synthetic data curriculum learning for low-data regimes; introduces digit-wise multi-label decomposition and achieves competitive accuracy with 10× fewer parameters
6. [[papers/2024/2024-jersey-number-recognition/2024-jersey-number-recognition|Jersey Number Recognition (CVPRW 2024)]] — the recognition-based successor; shows how pre-trained STR models surpass detection-based methods with minimal fine-tuning and cross-sport transfer
