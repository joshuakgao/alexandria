...
*Last updated: 2026-06-24 | Sources: 3 papers*

Simulation environments provide the standardized interfaces through which agents interact with virtual worlds for training and evaluation. Ha & Schmidhuber's World Models (NeurIPS 2018) demonstrated that learned neural networks can serve as simulation environments themselves — the MDN-RNN generates hallucinated latent-space environments (CarRacing, VizDoom) that agents can train inside, with policies transferring back to reality. This established the paradigm of "dream environments" as an alternative to hand-crafted simulators. Gymnasium (Towers et al., NeurIPS 2025) is the dominant API — the maintained successor to OpenAI Gym (used by World Models) with 18M+ installations — providing the Env and VectorEnv abstractions that underpin essentially all modern RL research. Its key innovations include separating termination from truncation (fixing widespread value estimation bugs), a functional API (FuncEnv) aligned with POMDP formalism for theoretical research and hardware acceleration, and first-class vectorized environments with multiple autoreset modes. The Gymnasium API has become the standard interface adopted by downstream systems including NitroGen's universal game simulator and numerous robotics platforms. Theory of Space (Zhang et al., ICLR 2026) demonstrates ThreeDWorld + Objaverse as a scalable platform for procedurally generated multi-room indoor environments with parallel text and vision modalities, designed specifically for evaluating active spatial exploration in foundation models.

## Trends

- **Learned environments as simulators**: World Models (2018) shows that learned dynamics models can replace hand-crafted simulators for policy training. The dream environment wraps an MDN-RNN with a standard gym.Env interface, making learned and hand-crafted environments interchangeable. Temperature control trades off realism vs. difficulty, with higher stochasticity producing more robust policies.
