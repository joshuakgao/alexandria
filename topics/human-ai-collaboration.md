...
## Trends
- **Grounded collaborative dialogue as a testbed**: The Minecraft Dialogue Corpus (Narayan-Chen et al., ACL 2019) established collaborative 3D construction as a benchmark for situated human-AI communication. The asymmetric Architect-Builder setup — one agent has privileged information (target structure), the other has privileged action capability (block placement) — forces genuine collaborative dialogue with error correction, clarification, and perspective-taking. Analysis revealed that human Architects devote over half their utterances to non-instruction acts (descriptions, confirmations, corrections), while seq2seq baselines generate instructions ~76% of the time regardless of context. Jayannavar et al. (ACL 2020) built the first automated Builder for this task, demonstrating that egocentric perspective transformation is essential for grounding Architect instructions — the Builder must interpret spatial references relative to its own viewpoint, not absolute coordinates. The model handles corrective dialogue (triggering removals after "sorry, my mistake") but cannot yet decide when to ask clarification questions, highlighting that the back-and-forth aspect of collaboration remains the hardest component. BAP v2 (Jayannavar et al., 2025) revisited the task with decomposed evaluation, revealing spatial reasoning as the primary bottleneck (Location F1 27.7 vs. Type 65.2 for the GRU baseline). Their Llama-CRAFTS model (53.0 F1) demonstrates that text-only LLMs can perform complex 3D spatial inference when given the Builder's egocentric frame as text input — but only when combined with synthetic spatial training data. Critically, LLMs trained solely on complex human collaborative dialogues fail on simpler synthetic spatial tasks, suggesting that naturalistic collaboration data alone does not teach robust spatial grounding.
...
- Bridging the dialogue act diversity gap: current models overwhelmingly generate instructions even when context calls for clarification, confirmation, or correction — achieving human-like pragmatic flexibility in situated dialogue remains open (Narayan-Chen et al., 2019).
- How to move from reactive instruction execution (Jayannavar et al., 2020) to proactive collaboration where the Builder agent decides when to act, when to ask for clarification, and when to propose alternatives — the full collaborative loop requires integrating action prediction with dialogue generation.
