...
Within the 3D scene understanding topic, OpenScene occupies the "semantic field" paradigm alongside LERF (NeRF-based), while Mask3D/OpenMask3D/Dual-Path represent the "instance mask" paradigm. HOV-SG (robotics topic) uses a similar multi-view CLIP feature fusion strategy at the segment level rather than the point level, with DBSCAN-based filtering to address the noise that per-point approaches suffer from.
